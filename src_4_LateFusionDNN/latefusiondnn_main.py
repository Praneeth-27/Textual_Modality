...
import has_valid_ex_trainer_latefusion
import ex_process_latefusiondnn
from ex_config import config_vars

if(len(sys.argv) == 1):
    EX_GROUP = 'DNNLATE2' #DNNLATE2
    EX_DATASET = 'HazumiThird' #IEMOCAP, Hazumi, IEMOCAP-6, etc.
    HAS_VALID = True
    MODALITY = 'ALL'
    FOLD_TRAIN = True
    FOLDS = 5
...
for modality_key in modality_group:
    ...
    for m in modality_key.split('+'):
        input_sizes[m] = data_dict[modality2data_key[m]][0].shape[-1]
    ...
    for fold in fold_group:
        ...
        if(config_vars['has_valid'] == False):
            train_dataloader, test_dataloader, label_weight_tensor = ex_process_latefusiondnn.split_fold_data_no_valid(data_dict, 
                                                                                                         ex_info_dict, 
                                                                                                         config_vars,
                                                                                                         model_key, 
                                                                                                         modality_key, 
            ...
            train_dataloader, valid_dataloader, test_dataloader, label_weight_tensor = ex_process_latefusiondnn.split_fold_data_has_valid(data_dict, 
                                                                                                                            ex_info_dict, 
                                                                                                                            config_vars,
                                                                                                                            model_key, 
                                                                                                                            modality_key, 
        ...
        configs = ex_config.configuration(modality_input_sizes=input_sizes, 
                                          ...
                                          fold_num=fold + 1, 
                                          random_seed=SEED)
        if(config_vars['ex_group'] == 'DNNLATE1'):
            model = dnn.DNN_late_fusion_1(configs)
        elif(config_vars['ex_group'] == 'DNNLATE2'):
            model = dnn.DNN_late_fusion_2(configs)
            
        if(configs.GPU_USE):
            model = model.cuda()
        
        if(config_vars['has_valid'] == True):
            result_dict = has_valid_ex_trainer_latefusion.train_process(train_dataloader, valid_dataloader, test_dataloader, label_weight_tensor, model, ex_info_dict, 
                                              configs, 0, GPU_USE)
