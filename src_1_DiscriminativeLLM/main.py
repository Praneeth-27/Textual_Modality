# -*- coding: utf-8 -*-
"""
This script is for runing main experiments of discriminative LLMs
"""
...
if(len(sys.argv) == 1): # you can run this script in IDE by setting configs here
    EX_GROUP = 'RoBerta' #[Freeze] Bert, RoBerta
    EX_DATASET = 'HazumiThird' #Hazumi, Hazumi-Paragraph, HazumiThird, HazumiThird-Paragraph
    HAS_VALID = True
    MODALITY = 'ALL'
    POOLING = 'CLS'
    FOLD_TRAIN = True
...
if(config_vars['ex_group'][:6] == 'Freeze'):
    ...
    with open(os.path.join(data_path, 'Data_DataDictModalityText.pkl'), 'rb') as pkl_i:
        data_dict = pickle.load(pkl_i)
...
save_path = os.path.join(save_root, result_dir_head + config_vars['ex_group'] + '_' + config_vars['ex_dataset'] + '_LR_' + str(config_vars['lr']) + \
                         '_Pooling' + str(config_vars['pooling']) + \
...
modality_all = list(data_dict['modality_text'][model_key].keys())
...
for modality_key in modality_group:
    ...
    for fold in fold_group:
        ...
        if(config_vars['has_valid'] == False):
            train_dataloader, test_dataloader, label_weight_tensor = ex_process.split_fold_data_no_valid(data_dict, 
                                                                                                         ex_info_dict, 
                                                                                                         config_vars,
                                                                                                         model_key, 
                                                                                                         modality_key, 
            ...
            train_dataloader, valid_dataloader, test_dataloader, label_weight_tensor = ex_process.split_fold_data_has_valid(data_dict, 
                                                                                                                            ex_info_dict, 
                                                                                                                            config_vars,
                                                                                                                            model_key, 
                                                                                                                            modality_key, 
        ...
        configs = ex_config.configuration(input_size=768, #equal to Bert or RoBerta's output size, but not adapatble for others 
                                          ...
                                          random_seed=SEED)
        if(config_vars['ex_group'][:6] == 'Freeze'):
            model = dnn.tanh_DNN(configs)
        else:
            model = pretrain_with_dnn_head.pre_train_model_with_DNN_head(configs)
