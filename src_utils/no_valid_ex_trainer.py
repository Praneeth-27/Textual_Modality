# -*- coding: utf-8 -*-
"""
This scrip is the training process for models except late fusion without validations
"""
...
def train_process(train_dataloader, test_dataloader, label_weights, model, ex_info_dict, 
    ...
    result_dict = {}
    for key in ['test_acc', 'test_weighted_f1', 'test_macro_f1']:
        result_dict[key] = 0
    for key in ['test_pred_list', 'test_true_list']:
        result_dict[key] = []
    ...
    for ep in range(configs.epoch):
        if(configs.ex_group[:6] == 'Freeze' or configs.ex_group in ['DNN']):
            if(ep <= 10):
                lrr = 0.001
            elif(ep <= 50):
                lrr = 0.001
                ...
                lrr = 0.0005
        if(configs.ex_group in ['Bert', 'RoBerta']):
            if(ep < 5):
                lrr = configs.lr
            else:
                lrr = 2e-5
        ...
        acc, weighted_f1, macro_f1, pred_list, true_list = train_step(model, pbar_train, pbar_test, criteria, 
        ...
        if(acc > best_acc):
            ...
            result_dict['test_acc'] = acc
            result_dict['test_weighted_f1'] = weighted_f1
            result_dict['test_macro_f1'] = macro_f1
            result_dict['test_pred_list'] = pred_list
            result_dict['test_true_list'] = true_list
    ...
    with open(configs.metric_result_save_path, 'w') as f_o:
        f_o.write('Fold: ' + str(configs.fold_num) + ' | ' + 'Test results: Acc : ' + str(round(best_acc, 5)) + ' | Weighted F1 : ' + str(round(best_weighted_f1, 5)) + ' | Macro F1 : ' + str(round(best_f1, 5)))
    with open(configs.result_save_path, 'wb') as pkl_o:
        pickle.dump(result_dict, pkl_o)
    return result_dict
    
def train_step(model, pbar_train, pbar_test, criteria, optimizer, configs, logger, ep, sequence_flag=0, use_label='da'):
    model.train()
    loss_list = []
    for i, (x_data, label) in pbar_train:
        if(configs.GPU_USE):
            if(configs.ex_group[:6] == 'Freeze' or configs.ex_group in ['DNN']):
                x_data = x_data.cuda()
            elif(configs.ex_group in ['Bert', 'RoBerta']):
                x_data = model.pretrained_tokenizer(x_data, return_tensors='pt', padding='longest')
                for key in x_data:
    ...
    acc, weighted_f1, macro_f1, pred_list, true_list = valid_test_process(model, pbar_test, configs, sequence_flag=sequence_flag, use_label=use_label)
